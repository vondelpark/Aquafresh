 ...
 *   awaiting_photo    → waiting for photo of the boat
 *   awaiting_name     → waiting for customer name
 *   awaiting_notes    → waiting for special instructions (optional)
 *   awaiting_confirm  → quote sent, waiting for yes/no
 *   booked            → payment link sent; booking committed on payment
...
async function handleMessage(msg) {
  ...
  if (inputLower === 'diensten' || inputLower === 'services' || inputLower === 'info') {
    const tierList = await pricing.tierListText();
    await wa.sendText(from,
      '*Onze diensten:*\n\n' + tierList +
  ...
  switch (conv.state) {
    ...
    case 'awaiting_location':
      ...
      break;
    case 'awaiting_photo':
      await handlePhoto(from, conv, input, msg);
      break;
    case 'awaiting_name':
      await handleName(from, conv, input);
      break;
    case 'awaiting_notes':
      await handleNotes(from, conv, input, buttonId);
      break;
...
async function handleWidth(from, conv, input) {
  ...
  const rates = await pricing.getRates();
  await wa.sendButtons(from,
    `${conv.data.boat_length_m}m × ${num}m = *${area} m²*. Welke service wil je?`,
    [
      { id: 'tier_basic', title: `Basic €${rates.basic.toFixed(2)}/m²` },
      { id: 'tier_extra', title: `Extra €${rates.extra.toFixed(2)}/m²` },
      { id: 'tier_heavy', title: `Heavy €${rates.heavy.toFixed(2)}/m²` },
    ]
  );
}
...
async function handleDate(from, conv, input) {
  ...
  // Check availability in Google Calendar for both slots
  let morningFree = true;
  let afternoonFree = true;
  try {
    const [m, a] = await Promise.all([
      calendar.checkAvailability(parsed, '08:00-12:00'),
      calendar.checkAvailability(parsed, '12:00-17:00'),
    ]);
    morningFree = m.available;
    afternoonFree = a.available;
  } catch (err) {
  ...
  if (!morningFree && !afternoonFree) {
    await wa.sendText(from,
      `Helaas, *${parsed}* is volgeboekt. 😕\n\nKies alsjeblieft een andere datum.`
    );
    return;
  }
  ...
  const buttons = [];
  if (morningFree) buttons.push({ id: 'time_morning', title: 'Ochtend (8-12u)' });
  if (afternoonFree) buttons.push({ id: 'time_afternoon', title: 'Middag (12-17u)' });
  buttons.push({ id: 'time_flexible', title: 'Flexibel' });

  let bodyText = `Datum: *${parsed}*.`;
  if (!morningFree) bodyText += ' (Ochtend is al bezet.)';
  if (!afternoonFree) bodyText += ' (Middag is al bezet.)';
  bodyText += ' Welk tijdslot heeft je voorkeur?';

  await wa.sendButtons(from, bodyText, buttons);
}
...
async function handleTime(from, conv, input, buttonId) {
  ...
  await wa.sendText(from,
    'Waar ligt je boot? 📍\n\n' +
    'Deel je *locatie-pin* via WhatsApp:\n' +
    '1. Tik op 📎 (bijlage) onderaan\n' +
    '2. Tik op *Locatie*\n' +
    '3. Zoek de jachthaven of verplaats de pin naar je boot\n' +
    '4. Tik op *Verstuur*'
  );
}

/**
 * Handle location — only accepts a WhatsApp location pin (required).
 */
async function handleLocation(from, conv, input, msg) {
  if (msg.location) {
    ...
    await wa.sendText(from,
      ...
      '📸 Stuur nu een *foto van je boot* zodat we de staat kunnen beoordelen.\n\n' +
      'Tik op 📎 (bijlage) → *Camera* of *Galerij* → maak of kies een foto en verstuur.'
    );
    return;
  }

  // No location pin received — keep asking
  await wa.sendText(from,
    'We hebben je *locatie-pin* nodig om je boot te vinden. 📍\n\n' +
    'Zo doe je dat:\n' +
    '1. Tik op 📎 (bijlage) onderaan\n' +
    '2. Tik op *Locatie*\n' +
    '3. Zoek de jachthaven of verplaats de pin naar je boot\n' +
    '4. Tik op *Verstuur*\n\n' +
    'Een getypte locatie is helaas niet voldoende — we hebben de exacte pin nodig.'
  );
}

/**
 * Handle photo — accepts a WhatsApp image message.
 */
async function handlePhoto(from, conv, input, msg) {
  if (msg.image) {
    conv.data.boat_photo_id = msg.image.id;
    conv.data.boat_photo_mime = msg.image.mime_type;
    if (msg.image.caption) conv.data.boat_photo_caption = msg.image.caption;

    conv.state = 'awaiting_name';
    await store.setConversation(from, conv);

    await wa.sendText(from,
      '📸 Foto ontvangen, bedankt!\n\nBijna klaar! Wat is je naam?'
    );
    return;
  }
  ...
  // Not a photo
  await wa.sendText(from,
    'Stuur alsjeblieft een *foto* van je boot. Dit is nodig zodat we de staat kunnen beoordelen.\n\n' +
    'Tik op 📎 (bijlage) → *Camera* of *Galerij* → verstuur de foto.'
  );
}
...
async function handleName(from, conv, input) {
  ...
  await wa.sendButtons(from,
    `Dankjewel, ${input}! 📝\n\nHeb je nog *speciale instructies of opmerkingen*? ` +
    `(bijv. toegangscode, ligplaatsnummer, bijzonderheden over de boot)\n\n` +
    `Typ je opmerking, of sla deze stap over.`,
    [{ id: 'notes_skip', title: 'Geen opmerkingen' }]
  );
}

async function handleNotes(from, conv, input, buttonId) {
  if (buttonId === 'notes_skip' || /^(geen|nee|no|skip|-)\.?$/i.test(input)) {
    conv.data.notes = null;
  } else if (input === '[image]' || input === '[media]' || input === '[location]') {
    await wa.sendText(from,
      'Typ je opmerking als tekstbericht, of tik op *Geen opmerkingen*.'
    );
    return;
  } else {
    conv.data.notes = input;
  }

  // Calculate quote using length × width
  const quote = await pricing.calculateQuote(
  ...
  const summary =
    ...
    `${locationLine}\n` +
    `📸 Foto: ontvangen\n` +
    (conv.data.notes ? `📝 Opmerkingen: ${conv.data.notes}\n` : '') +
...
async function handleConfirmation(from, conv, input, buttonId) {
  ...
  if (buttonId === 'confirm_yes' || /ja|yes|ok|bevestig|confirm|akkoord/i.test(input)) {
    // Create booking (committed only after payment)
    ...
    const booking = {
      ...
      boat_photo_id: conv.data.boat_photo_id || null,
      boat_photo_mime: conv.data.boat_photo_mime || null,
      notes: conv.data.notes || null,
    ...
    if (booking.payment_url) {
      confirmMsg += `\n💳 *Betaal hier om je boeking te bevestigen:*\n${booking.payment_url}\n\n` +
        `Zodra de betaling binnen is, staat je afspraak definitief vast en ontvang je een bevestiging. ✅`;
    } else {
      confirmMsg += `\nWe sturen je zo een betaallink. Na betaling is je boeking definitief.`;
    }
