...
const tikkie = require('./tikkie');

/**
 * Process an incoming message and advance the conversation.
 * @param {{ from: string, text: string, buttonId: string, messageId: string }} msg
 */
async function handleMessage(msg) {
  ...
  switch (conv.state) {
    ...
    case 'awaiting_location':
      await handleLocation(from, conv, input);
      break;
    case 'awaiting_name':
...
async function handleLength(from, conv, input) {
  const num = parseFloat(input.replace(',', '.').replace(/[^0-9.]/g, ''));
  if (!num || num < 1 || num > 60) {
    await wa.sendText(from,
  ...
  await wa.sendButtons(from,
    `${num} meter, begrepen! Welke service wil je?`,
    [
...
async function handleTime(from, conv, input, buttonId) {
  ...
  if (buttonId === 'time_morning' || /ochtend|morning|8.*12/i.test(input)) {
    ...
    // Try to use their text as-is if it looks like a time
  ...
  await wa.sendText(from,
    'Waar ligt je boot? Stuur de jachthaven/ligplaats of deel je locatie-pin. 📍'
  );
}

async function handleLocation(from, conv, input) {
  if (input.length < 2) {
    await wa.sendText(from,
      'Stuur alsjeblieft de naam van de jachthaven of ligplaats waar je boot ligt.'
    );
    return;
  }
...
async function handleName(from, conv, input) {
  ...
  // Calculate quote
  const quote = pricing.calculateQuote(conv.data.boat_length_m, conv.data.service_tier);
...
async function handleConfirmation(from, conv, input, buttonId) {
  ...
  if (buttonId === 'confirm_yes' || /ja|yes|ok|bevestig|confirm|akkoord/i.test(input)) {
    ...
    try {
      ...
      // Continue without calendar — not a blocker
    }

    // Try to generate Tikkie payment link
    try {
      const tikkieUrl = await tikkie.createPaymentRequest(booking);
      booking.tikkie_url = tikkieUrl;
    } catch (err) {
      console.error('[Tikkie Error]', err.message);
      // Continue without Tikkie — owner can send manually
    }
    ...
    if (booking.tikkie_url) {
      confirmMsg += `\n💳 Betaal via Tikkie:\n${booking.tikkie_url}\n\nNa betaling is je boeking definitief.`;
    } else {
      confirmMsg += `\nWe sturen je zo een betaallink via Tikkie.`;
    }
...
/**
 * Very simple date parser — handles common Dutch/international formats.
...
function parseDate(input) {
  ...
  // Relative: morgen, overmorgen, volgende week
  ...
  if (vwMatch && weekDays[vwMatch[1]] !== undefined) {
    ...
    if (daysAhead <= 7) daysAhead += 7; // ensure it's next week
