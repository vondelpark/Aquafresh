 ...
 *
 * Surface area = boat_length_m × area_factor (default 2.5)
 ...
 * Accepts "basic", "Basic", "BASIC", "extra", "heavy duty", "heavy", "heavyduty" etc.
 */
 ...
 */
function calculateQuote(boatLengthM, tierKey) {
  const tier = TIERS[tierKey];
  if (!tier) throw new Error(`Unknown tier: ${tierKey}`);

  const area = Math.round(boatLengthM * AREA_FACTOR * 10) / 10;
...
module.exports = { TIERS, AREA_FACTOR, MINIMUM_PRICE, normalizeTier, calculateQuote, tierListText };
